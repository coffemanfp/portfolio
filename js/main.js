...
(function(html) {
    ...
    * -------------------------------------------------- */
	const tl = anime.timeline( {
		...
		.add({
			targets: "#greeting_entity",
			complete: function(anim) {
				document.querySelector("#greeting_entity").onmouseover = event => {
				    applyGlitchEffect(event.target, entities[entitiesTurn]);
				}

				setInterval(() => {
					applyGlitchEffect(document.querySelector("#greeting_entity"), entities[entitiesTurn]);
				}, 3000);
			}});
			...
			blocks.forEach(function(current) {
				...
				if (inView && (!isAnimated)) {
					anime({
						...
						opacity: [0, 1],
						translateY: [100, 0],
						// delay: anime.stagger(0, {start: 200}),
						duration: 800,
						easing: "easeInOutCubic",
						begin: function(anim) {
